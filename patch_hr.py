...
"""
content = re.sub(r'(async function processJobSearchInsight\s*\(\s*emails\s*\)\s*\{)', new_func + r'\n\1', content)
...
content = re.sub(r'hr:\s*\{\s*total:.*?\},', 'hr: get().hrStats,', content, flags=re.DOTALL)


with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

print("useEmailStore HR patches applied.")

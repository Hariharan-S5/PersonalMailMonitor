...
export const emailService = {
  ...
  getEmails: async (accessToken, userEmail, customQuery = null, detailLimit = 20, listLimit = 100) => {
    ...
    const listResponse = await fetchWithRetry(`${GMAIL_API_BASE}/messages?maxResults=${listLimit}${queryParam}`, {
      headers: { Authorization: `Bearer ${accessToken}` }
    });
  ...
  getSecurityAlerts: async (accessToken) => {
    ...
    for (const msg of data.messages) {
      const dResp = await fetchWithRetry(`${GMAIL_API_BASE}/messages/${msg.id}?format=minimal`, {
        headers: { Authorization: `Bearer ${accessToken}` }
      });
      if (dResp.ok) {
        details.push(await dResp.json());
      }
      await wait(50);
    }
    return details;

...
export default function VerificationIntro() {
  const { verifySubscription, applyLuckyCoupon, userPermissions, fetchUserPermissions, createNewUserRecord } = useEmailStore();
  ...
  const submitCoupon = (e) => {
    ...
    setTimeout(() => {
      if (applyLuckyCoupon(coupon)) setCouponOk(true);
      else { setCouponErr('Invalid or expired code.'); setCouponLoading(false); }
    }, 1200);
  };

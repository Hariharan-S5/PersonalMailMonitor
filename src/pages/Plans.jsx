...
const Plans = () => {
  ...
  const { subscriptionType, upgradeSubscription, verificationWay } = useEmailStore();
  ...
  const tiers = [
    {
      ...
      buttonText: subscriptionType === 'Basic' ? 'Current Plan' : (verificationWay === 'lucky' ? 'Bypass Active' : 'Standard Node'),
    ...
    },
    {
      ...
      buttonText: subscriptionType === 'Pro' ? 'Current Plan' : (verificationWay === 'lucky' ? 'Bypass Active' : 'Select Pro'),
    ...
    },
    {
      ...
      buttonText: subscriptionType === 'Elite' ? 'Current Plan' : (verificationWay === 'lucky' ? 'Elite Protocol' : 'Go Elite'),
          ...
          {tiers.map((tier, idx) => (
            ...
            >
              ...
              <motion.button 
                ...
                disabled={tier.isCurrent || verificationWay === 'lucky'}
                className={`w-full py-4 rounded-[1.5rem] font-black text-xs uppercase tracking-widest transition-all duration-300 relative z-10 border shadow-lg flex items-center justify-center gap-3 ${(tier.isCurrent || verificationWay === 'lucky') ? "opacity-50 cursor-not-allowed" : "hover:scale-[1.03] active:scale-95"} ${tier.buttonClass}`}
              >
                {tier.buttonText}

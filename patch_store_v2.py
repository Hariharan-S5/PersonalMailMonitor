...
"""
# Escape backslashes for re.sub replacement
new_func_escaped = new_func.replace('\\', '\\\\')

content = re.sub(r'(async function processPaymentAppInsight\s*\(\s*emails\s*\)\s*\{)', new_func_escaped + r'\1', content)
...
from_text = """processPaymentAppInsight(paymentAppEmailDetails)
      ]);"""
to_text = """processPaymentAppInsight(paymentAppEmailDetails),
...
from_text2 = """processPaymentAppInsight(paymentAppEmailDetails)
      ]);"""
to_text2 = """processPaymentAppInsight(paymentAppEmailDetails),
        processJobSearchInsight([])
      ]);"""
content = content.replace(from_text2, to_text2)

from_text3 = """paymentAppSpend: newPaymentAppSpend,
        error: null"""
to_text3 = """paymentAppSpend: newPaymentAppSpend,
        jobSearchStats: newJobSearchStats,
        error: null"""
content = content.replace(from_text3, to_text3)
